...
from pqnstack.base.driver import DeviceClass, DeviceDriver, DeviceInfo, DeviceStatus, log_operation, log_parameter
...
class PM100DDevice(DeviceDriver):
    ...
    def start(self) -> None:
        """Connect to the PM100D via USBTMC and prepare for readings."""
        try:
            ...
            logger.error("PM100D connect failed at %s: %s", self.address, exc)
            raise
    ...
    def wavelength(self) -> float:
        """Get the operating wavelength (SCPI SENSE:CORRECTION:WAVELENGTH?)."""
        if self._device is None:
            raise DeviceNotStartedError("Start the PM100D before getting wavelength")
