...
def measure_tomography_raw(idler_hwp, idler_qwp,
                        signal_hwp, signal_qwp,
                        timetagger, measurement_duration, binwidth = 100,
    ...
    for i in range(6):
        for j in range(6):
            ...
            sleep(3)
            tomography_results.append(timetagger.measure_coincidence(channel1, channel2, binwidth, int(measurement_duration * 1e12)))
            singles_counts.append(timetagger.measure_countrate([channel1, channel2], int(measurement_duration * 1e12])))

    current_time = datetime.datetime.now().isoformat()
    results = {
        "timestamp": current_time,
        "tomography_raw_counts": tomography_results,
