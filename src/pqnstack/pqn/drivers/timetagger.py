...
class TimeTaggerDevice(DeviceDriver):
    """
    Abstract base class for a time tagger device. Inherit from this class
    to implement specialized time tagger drivers, such as one for Swabian
    ...
    def measure_coincidence(
        ...
        groups, for the specified real-time measurement duration and
        coincidence window.
        """
...
class SwabianTimeTagger(TimeTaggerDevice):
    ...
    def info(self) -> TimeTaggerInfo:
        """
        Returns information about this device, including its current parameters
    ...
    def set_test_signal(self, channels: List[int], enable: bool = True, divider: Optional[int] = None) -> None:
